...
interface HeroBannersCustomizerProps {
  config: HomepageConfig;
  onChange: (config: HomepageConfig) => void;
}

export default function HeroBannersCustomizer({ config, onChange }: HeroBannersCustomizerProps) {
  const handleToggleBannerActive = (id: string) => {
    const updated = { ...config };
    updated.heroBanners = updated.heroBanners.map((b: Banner) =>
  ...
  const handleRemoveBanner = (id: string) => {
    const updated = { ...config };
    updated.heroBanners = updated.heroBanners.filter((b: Banner) => b.id !== id);
    onChange(updated);
  };

  const handleMoveBanner = (index: number, direction: 'UP' | 'DOWN') => {
    const updated = { ...config };
    const banners = [...updated.heroBanners];
    ...
    <Flex direction="column" gap="4">
      ...
      {config.heroBanners.length === 0 ? (
        ...
        <Grid columns={{ initial: '1', sm: '2' }} gap="4">
          {config.heroBanners.map((banner: Banner, index: number) => (
