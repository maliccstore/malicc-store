...
function FilterSortContent({ onApply, currentResultCount }: FilterSortProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      trackEvent({
        event: ANALYTICS_EVENTS.PRODUCT_SEARCH,
        sessionId: getSessionId(),
        userId: user?.id,
        metadata: {
          searchQuery: search.trim(),
          resultCount: currentResultCount ?? 0,
        },
      });
    }, 800);

    return () => clearTimeout(timer);
  }, [search, user?.id, searchParams, currentResultCount]);
  ...
  const handleApply = () => {
    ...
    // Track PRODUCT_SORT if sort changed
    if (sort !== prev.sort) {
      trackEvent({
        event: ANALYTICS_EVENTS.PRODUCT_SORT,
        sessionId,
        userId,
        metadata: { sortField: sort },
      });
    }

    // Track PRODUCT_FILTER if any filter changed
    if (
      ...
      trackEvent({
        event: ANALYTICS_EVENTS.PRODUCT_FILTER,
        sessionId,
        userId,
        metadata: {
          appliedFilters: {
